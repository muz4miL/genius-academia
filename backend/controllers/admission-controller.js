...
const getPendingSignups = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ message: "Error fetching signups", error: err.message });
  }
};
  ...
  try {
    ...
    if (!assignedClass || !sessionId || !rollNumber || !admissionFee) {
      return res.status(400).json({
        message:
          "assignedClass, sessionId, rollNumber, and admissionFee are required",
      });
    }
    ...
    res
      .status(500)
      .json({ message: "Error approving signup", error: err.message });
  }
};
...
const promoteToStudent = async (req, res) => {
  try {
    ...
    if (admission.student) {
      return res
        .status(400)
        .json({ message: "Student already created for this admission" });
    }
    ...
    try {
      ...
      // NOTE: Password will be hashed by Student model pre-save hook
      // Do NOT pre-hash here — the model handles it automatically
    ...
    res
      .status(500)
      .json({ message: "Error promoting to student", error: err.message });
  }
};
